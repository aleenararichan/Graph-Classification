...
model_g.fit_generator(gen(train, node_int_mapping), validation_data=gen(test, node_int_mapping), epochs=400, verbose=2,
...
model = get_features_graph_model(n_features=n_features, n_classes=int(max(Y_train) + 1), n_nodes = len(node_int_mapping))
...
model.fit([X_train, X_train_2, X_G_train], Y_train, validation_data=([X_val, X_val_2, X_G_val], Y_val),
          epochs=5000, verbose=2, callbacks=[early])
